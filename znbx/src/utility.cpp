...
int FNodeBuilder::CreateSeg(int linenum, int sidenum) {
	...
	seg.angle = PointToAngle(
		Vertices[seg.v2].x - Vertices[seg.v1].x, Vertices[seg.v2].y - Vertices[seg.v1].y
	);
	...
	D(printf(
		"Seg %4d: From line %d, side %s (%5d,%5d)-(%5d,%5d)  [%08x,%08x]-[%08x,%08x]\n", segnum,
		linenum, sidenum ? "back " : "front", Vertices[seg.v1].x >> 16, Vertices[seg.v1].y >> 16,
		Vertices[seg.v2].x >> 16, Vertices[seg.v2].y >> 16, Vertices[seg.v1].x, Vertices[seg.v1].y,
...
void FNodeBuilder::GroupSegPlanes() {
	...
	for (i = planenum = 0; i < (int)Segs.Size(); ++i) {
		...
		if (check != NULL) {
			...
			FSimpleLine pline = { Vertices[seg->v1].x, Vertices[seg->v1].y,
		...
		if (GetPolyExtents(spot->polynum, bbox)) {
			...
			if (j < anchors.Size()) {
				...
				for (unsigned int j = 0; j < Segs.Size(); ++j) {
					...
					if ((v1->y < center.y && v2->y < center.y) ||
						(v1->y > center.y && v2->y > center.y)) { // Not crossed
						continue;
					}
				...
				if (closestdist != FIXED_MAX) {
					...
					P(Printf(
						"Found polyobj in sector %d (loop %d)\n", Segs[closestseg].frontsector,
...
int FNodeBuilder::MarkLoop(uint32_t firstseg, int loopnum) {
	...
	if (Segs[firstseg].loopnum != 0) { // already marked
		return loopnum;
	}
...
void FNodeBuilder::AddSegToBBox(znbx_I16F16 bbox[4], const FPrivSeg* seg) {
	...
	if (v1->x < bbox[BOXLEFT])
		bbox[BOXLEFT] = v1->x;
	if (v1->x > bbox[BOXRIGHT])
		bbox[BOXRIGHT] = v1->x;
	if (v1->y < bbox[BOXBOTTOM])
		bbox[BOXBOTTOM] = v1->y;
	if (v1->y > bbox[BOXTOP])
		bbox[BOXTOP] = v1->y;

	if (v2->x < bbox[BOXLEFT])
		bbox[BOXLEFT] = v2->x;
	if (v2->x > bbox[BOXRIGHT])
		bbox[BOXRIGHT] = v2->x;
	if (v2->y < bbox[BOXBOTTOM])
		bbox[BOXBOTTOM] = v2->y;
	if (v2->y > bbox[BOXTOP])
		bbox[BOXTOP] = v2->y;
}
...
int FNodeBuilder::FVertexMap::InsertVertex(FNodeBuilder::FPrivVert& vert) {
	...
	int blk[4] = { GetBlock(minx, miny), GetBlock(maxx, miny), GetBlock(minx, maxy),
				   GetBlock(maxx, maxy) };
	unsigned int blkcount[4] = { VertexGrid[blk[0]].Size(), VertexGrid[blk[1]].Size(),
								 VertexGrid[blk[2]].Size(), VertexGrid[blk[3]].Size() };
	for (int i = 0; i < 4; ++i) {
