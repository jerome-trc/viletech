...
FBlockmapBuilder::FBlockmapBuilder(FLevel& level) : Level(level) {
	BuildBlockmap();
}
...
int BoxOnSide(int bx1, int by1, int bx2, int by2, int lx1, int ly1, int lx2, int ly2) {
	int p1;
	int p2;
		...
		for (i = 0; i < stuff[2] * stuff[3]; ++i) {
			...
			while (stuff[i1] != -1) {
				...
				if (BlockMap[i2] == 0xffff) {
					...
					if (BoxOnSide(
							stuff[0] + 128 * x, stuff[1] + 128 * y, stuff[0] + 128 * x + 127,
							stuff[1] + 128 * y + 127, Vertices[Lines[l].v1].x,
							Vertices[Lines[l].v1].y, Vertices[Lines[l].v2].x,
							Vertices[Lines[l].v2].y
						)) {
						printf(
							"not in cell %4d: line %4d [%2d,%2d] : (%5d,%5d)-(%5d,%5d)\n", i,
							stuff[i1], x, y, stuff[0] + 128 * x, stuff[1] + 128 * y,
			...
			while (BlockMap[i1] != 0xffff) {
				...
				if (stuff[i2] == -1) {
					...
					if (!BoxOnSide(
						...
						printf(
							"EXT in cell %4d: line %4d [%2d,%2d] : (%5d,%5d)-(%5d,%5d)\n", i,
							(short)BlockMap[i1], x, y, (short)BlockMap[0] + 128 * x,
							(short)BlockMap[1] + 128 * y, (short)BlockMap[0] + 128 * x + 127,
							(short)BlockMap[1] + 128 * y + 127
						);
					}
				}
				i1++;
