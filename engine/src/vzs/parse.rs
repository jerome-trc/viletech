...
use doomfront::{rowan::GreenNode, util::builder::{GreenCache, GreenCacheNoop}};
...
/// into a token tagged [`Syn::Unknown`](crate::vzs::Syn::Unknown).
#[must_use]
pub fn parse_file<C: GreenCache>(_source: &str, _path: impl AsRef<VPath>, _: Option<C>) -> Option<FileParseTree> {
	todo!()
}

#[must_use]
pub fn parse_repl<C: GreenCache>(_source: &str, _: Option<C>) -> Option<ParseTree> {
	todo!()
}
		...
		while let Some(dir_path) = dir_q.pop() {
			...
			dir.child_paths()
				...
				.for_each(|child_path| {
					...
					if let Some(ptree) = parse_file::<GreenCacheNoop>(child.read_str(), child_path, None) {
