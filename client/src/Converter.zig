...
pub fn run(args: Args) !void {
    ...
    if (std.mem.eql(u8, args.in, "-")) {
        ...
        if (ContentId.by_basename.get(std.fs.path.basename(path))) |c_id| {
            in_format = c_id;
...
pub const ContentId = enum {
    ...
    pub const by_basename = std.StaticStringMapWithEql(
        ContentId,
        std.ascii.eqlIgnoreCase,
    ).initComptime(.{
        .{ "decohack", ContentId.decohack },
        .{ "dehacked", ContentId.dehacked },
    });

    pub const by_extension = std.StaticStringMap(ContentId).initComptime(.{
        .{ ".deh", ContentId.dehacked },
        .{ ".dh", ContentId.decohack },
        .{ ".mus", ContentId.dmx_mus },
        .{ ".mid", ContentId.midi },
    });

    pub fn prettyName(self: ContentId) []const u8 {
        return switch (self) {
            ContentId.unknown => "Unknown",
            ContentId.decohack => "DECOHack",
            ContentId.dehacked => "DeHackEd",
            ContentId.dmx_mus => "DMX MUS",
            ContentId.midi => "MIDI",
        };
    }
};
