...
void AM_initPlayerTrail(CCore* _cx)
{
    (void)_cx;
  trail_index = -1;
  trail_size = 0;
...
static void AM_HighlightByTag(CCore* cx)
{
  fixed_t x, y;
  sector_t *sec;
  line_t *line;
  ...
  if (!repeat || (!highlight.sec && !highlight.line))
  {
    ...
    doom_printf(cx, "Highlight sector %d, tag %d\n", highlight.sec->iSectorID, sec->tag);
  }
  else if (highlight.sec)
  {
    ...
    doom_printf(cx, "Highlight line %d, tag %d\n", highlight.line->iLineID, line->tag);
  }
  else
  {
    highlight.line = NULL;
    ...
    doom_printf(cx, "Highlight nothing\n");
  }
...
dboolean AM_Responder(CCore* cx, event_t* ev) {
  ...
  else if (dsda_InputActivated(dsda_input_map_follow))
  {
    ...
    dsda_AddMessage(cx, automap_follow ? s_AMSTR_FOLLOWON : s_AMSTR_FOLLOWOFF);
  ...
  else if (dsda_InputActivated(dsda_input_map_grid))
  {
    ...
    dsda_AddMessage(cx, automap_grid ? s_AMSTR_GRIDON : s_AMSTR_GRIDOFF);
  ...
  {
    AM_clearMarks(cx);  // Ty 03/27/98 - *not* externalized
    dsda_AddMessage(cx, s_AMSTR_MARKSCLEARED);
  ...
  else if (dsda_InputActivated(dsda_input_map_rotate))
  {
    ...
    dsda_AddMessage(cx, automap_rotate ? s_AMSTR_ROTATEON : s_AMSTR_ROTATEOFF);
  ...
  else if (dsda_InputActivated(dsda_input_map_textured))
  {
    ...
    dsda_AddMessage(cx, map_textured ? s_AMSTR_TEXTUREDON : s_AMSTR_TEXTUREDOFF);
  ...
  else if (dsda_InputActivated(dsda_input_map_highlight_by_tag))
  {
    if (!dsda_RevealAutomap())
      doom_printf(cx, "Highlight requires iddt");
    else
      AM_HighlightByTag(cx);
