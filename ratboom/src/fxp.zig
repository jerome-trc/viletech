...
pub fn Fxp(fracbits: comptime_int) type {
    ...
    return packed struct {
        ...
        pub fn fromInt(int: Backing) Self {
            return .{ .inner = @shlWithOverflow(int, fracbits).@"0" };
        }

        pub fn fromF64(float: f64) Self {
            const i = std.math.lossyCast(Backing, float);
            return .{ .inner = i * (1 << fracbits) };
        }
