...
{
  snprintf(buf,sz,"%s v%s (https://github.com/jerome-trc/viletech/)",PACKAGE_NAME,PACKAGE_VERSION);
  return buf;
}
...
char* I_FindFileInternal(const char* wfname, const char* ext, dboolean isStatic)
{
  ...
  static struct {
    ...
    {VTEC_WAD_DIR}, // build-time configured DOOMWADDIR
    {VTEC_ABSOLUTE_PWAD_PATH}, // build-time configured absolute path to dsda-doom.wad
